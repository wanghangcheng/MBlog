...
<div class="login">
    ...
    <form action="reg" method="post">
        ...
        <div id="id_nickname">
            <input maxlength="9" class="form-control border" name="nickname" value="" placeholder="昵称" type="text" data-required>
        </div>
        <label for="id_email">邮箱:</label>
        <div id="id_email"> 
            <input maxlength="64" class="form-control border" id="email" name="email" value="" placeholder="邮箱地址" type="text" data-required data-conditional="email" data-description="email" data-describedby="message">
        ...
        <div id="id_password">
            <input id="password" class="form-control border" maxlength="18" name="password" placeholder="密码" type="password" >
        </div>
        <label for="id_password2">确认密码:</label>
        <div id="id_password2">
            <input maxlength="18" class="form-control border" name="password2" placeholder="请再次输入密码" type="password" data-required data-conditional="confirm" data-describedby="message" data-description="confirm">
        </div>
            ...
            submitHandler: function(form){
            	...
            	$.ajax({ 
            		...
            		success: function(data){
                    	layer.close(layer1);
                    	if(data.code==100){
    						layer.msg('用户名已被占用', function(){});
    					}
    					if(data.code==101){
    						layer.msg('邮箱已被占用', function(){});
    					}
    					if(data.code==102){
    						layer.msg('昵称已被占用', function(){});
    					}
    					if(data.code==200){
    						layer.msg('恭喜，注册成功，2秒后为你跳转登录页面', {icon: 6});
    						setTimeout(function(){
